        ...
        <div v-else class='w-full max-w-[700px] my-8 border border-grey-100 dark:border-grey-500 rounded-md shadow-md'>
            <UAccordion color="gray" variant="soft" size="lg" :items="items">
                <template #subItems="{ item }">
                    <div v-for='oneItem in item?.content' :key='oneItem.id' class="">
                        <div class='flex w-full justify-between px-4'>
                            <div className='flex flex-col mb-2'>
                                <p className='font-medium text-grey-800 dark:text-grey-200'>
                                    {{ oneItem.title }}
                                </p>
                            ...
                            <p className='mb-2 whitespace-nowrap text-grey-800 dark:text-grey-200'>
